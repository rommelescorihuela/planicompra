...
<style>
.titulo {margin-left:70px;margin-top: 90px;}
.tg  {border-collapse:collapse;border-spacing:0;margin-left: 70px;margin-top: 100px;}
...
.tg td{border-color:black;border-style:solid;border-width:1px;font-family:Arial, sans-serif;font-size:18px;
  overflow:hidden;padding:33px 40px;word-break:normal;}
...
.poa {border-color:inherit;text-align:center;margin-top: 40px;}
...
<div class="titulo">
<h4>ACCIÓN CENTRALIADA: <?= $model->descripcion ?></h4> <h4>OBJETIVO: <?= $model->objetivo ?></h4><?php foreach($list2 as $lista2) {?><h4>UNIDAD EJECUTORA: <?= $lista2->idgerencia0->gerencia ?></h4><?php }?>
</div>
<div class="container">
<table class="tg">
<thead>
